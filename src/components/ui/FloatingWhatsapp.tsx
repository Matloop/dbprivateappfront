...
import { usePathname } from "next/navigation"; // <--- 1. Importe isso
const CONFIG = {
  PHONE: "554796510619",
  MESSAGE: "Olá! Vim pelo site da DB Private e gostaria de mais informações."
};
export function FloatingWhatsApp() {
  const pathname = usePathname(); // <--- 2. Pegue a rota atual

  // <--- 3. Adicione essa verificação no início do componente
  // Se estiver na intranet, retorna null (não renderiza nada)
  if (pathname && pathname.startsWith("/intranet")) {
    return null;
  }
  const link = `https://wa.me/${CONFIG.PHONE}?text=${encodeURIComponent(
    CONFIG.MESSAGE
  )}`;
