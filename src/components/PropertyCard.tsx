...
import { useRouter } from 'next/navigation'; // Mudou de react-router-dom
...
export const PropertyCard = ({ property }: { property: Property }) => {
  ...
  // Navegação do Carrossel (Previne abrir o link do imóvel)
  ...
  return (
    <Card 
      className="group relative flex flex-col overflow-hidden border-border bg-[#1a1a1a] transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-black/50 cursor-pointer"
      onClick={handleClick}
    >
      {/* IMAGEM */}
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-black group/image">
        ...
        {/* Setas (Só aparecem se tiver +1 foto e no hover) */}
        {images.length > 1 && (
          <>
            <button onClick={prevImage} className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-black/50 p-1.5 text-white opacity-0 transition-opacity hover:bg-primary hover:text-black group-hover/image:opacity-100">
              <ChevronLeft size={16} />
            </button>
            <button onClick={nextImage} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-black/50 p-1.5 text-white opacity-0 transition-opacity hover:bg-primary hover:text-black group-hover/image:opacity-100">
        ...
        <button 
          ...
          className="absolute top-3 right-3 rounded-full bg-black/40 p-2 text-white backdrop-blur-sm transition-colors hover:bg-white hover:text-red-500 z-10"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={favorite ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" className={`h-5 w-5 ${favorite ? "text-red-500" : ""}`}>
